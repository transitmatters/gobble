...
| `test_get_gtfs_archive_day_is_feed` | *(integration)* When the requested date is the feed start date, returns the archive directory named after that date |
| `test_get_gtfs_archive_day_not_feed` | *(integration)* When the requested date falls mid-feed, returns the archive directory named after the feed start date |
| `test_read_gtfs_date_exists_feed_is_read` | *(integration)* Reads a real GTFS archive and verifies Orange Line termini, Red Line stop data, and Route 1 bus trips to Harvard |
